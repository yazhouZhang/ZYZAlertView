...
#import "ZYZToastView.h"

@interface ZYZToastView()
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([self.titleText length] == 0)
    {
        ...
        CGSize labelSize = [_labelText sizeWithFont:self.label.font
                                  constrainedToSize:CGSizeMake(ZYZTipLabelMaxWidth, 1000)
    ...
    {
        //title的size
        CGSize titleSize = CGSizeMake(ZYZTipTitleMaxWidth, 20);
        self.label.font = [UIFont systemFontOfSize:16.0f];
        //计算文字的size
        CGSize size = CGSizeMake(ZYZTipTitleMaxWidth, 1000);
        CGSize labelSize = [self.label.text boundingRectWithSize:size options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:self.label.font} context:nil].size;
        CGSize maskSize = CGSizeMake(ZYZTipTitleMaxWidth+40, titleSize.height+labelSize.height+30);
